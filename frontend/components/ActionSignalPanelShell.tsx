 ...
 *
 * "Tavsiye Edilen Aksiyon" alanına [Race] / [Klasik] toggle ekler.
 * - Race modu : ActionSignalRaceLayer (lazy, default).
...
const ActionSignalRaceLayer = lazy(
  () => import("@/components/ActionSignalRaceLayer"),
);

type ViewMode = "race" | "classic";
    ...
    <div data-testid="action-signal-shell" className="w-full max-w-full min-w-0 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-1.5 mb-1.5 bg-eyay-raised/60 border border-eyay-border/60 rounded-xl">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <button
            type="button"
            onClick={() => { setDegradedMsg(null); setMode("race"); }}
      ...
      {mode === "race" && (
        ...
        >
          <Suspense fallback={legacyView}>
            <ActionSignalRaceLayer report={report} />
          </Suspense>
        </ActionSignalErrorBoundary>
      )}
    </div>
